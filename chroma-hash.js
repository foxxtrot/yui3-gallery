...
YUI.add('chroma-hash', function(Y) {
	...
	var configureNode = function(n, o) {
		var animations = [];
		...
		Y.Array.each(chromaHashesForElement(n), 
			function(el, i) { 
				var node = Y.get(el);
				animations[i] = new Y.Anim({ node: node }); 
			}
		);
		...
		n.on('keyup', function(e) {
			...
			var id = n.get('id'),
					md5 = hex_md5('' + n.get('value') + ':' + o.salt),
			...
			if (n.get('value').length < o.minimum) {
				Y.Array.each(animations,
					function(anim, i) {
						var y = anim.get('node');
						anim.stop();
						var g = (parseInt(col[i], 0x10) % 0xF).toString(0x10);
						Y.log(y.getStyle('backgroundColor'));
						Y.log('#' + g + g + g);
	...
	Y.mix(ChromaHash, {
		...
		add: function(node, config) {
			var c = config || { };
			c.bars = c.bars || 3;
			c.salt = c.salt || "7be82b35cb0199120eea35a4507c9acf";
			c.minimum = c.minimum || 6;
			if (c.bars < 1 || c.bars > 4) {
				Y.message("Bars argument must be between 1 and 4", "warn", "ChromaHash");
			} else {
				Y.all(node).each(function(n) {
					configureNode(n, config);
				});
			}
		},
		init: function() { }
...
}, "0.2", { requires: ['node', 'event', 'stylesheet', 'collection', 'anim-color'] });
